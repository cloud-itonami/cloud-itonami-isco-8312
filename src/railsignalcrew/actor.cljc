(ns railsignalcrew.actor
  "RailSignalCrewActor — the ISCO-08 8312 railway brake, signal and
  switch operator coordination actor as a `langgraph.graph/state-graph`
  ...
  The unconditional invariant: the RailSignalCrewAdvisor can never
  directly commit a record the RailSignalCrewGovernor refuses — every
  commit-record! call is gated behind `:decide`. This actor
  coordinates ADMINISTRATIVE / LOGISTICS SCHEDULING only: it never
  operates a switch or signal, and no maturity level or human approval
  path in this graph can commit a switch-throw, signal-clearance or
  track-routing finalization (`railsignalcrew.governor`'s rule 5 is a
  hard, permanent block with no `:request-approval` escape hatch)."
...
(defn build-graph
  "Build a compiled RailSignalCrewActor graph. `store` implements
  `railsignalcrew.store/Store`. `advisor` implements
  `railsignalcrew.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:operator-id (:operator-id request)
                                    :op (:op proposal)
                                    :section-id (:section-id proposal)
                                    :payload proposal}]
